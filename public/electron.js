...
log.transports.file.resolvePathFn = () =>
  path.join("/Users/apple/bluebill-autoupdate-on-mygit", "logs/main.log");
...
// IPC handler for checking updates and initiating the download
ipcMain.on('CHECK_FOR_UPDATE', (event) => {
  autoUpdater.checkForUpdates();
  event.returnValue = 'Checking for update';
});

ipcMain.on('DOWNLOAD_UPDATE', (event) => {
  autoUpdater.downloadUpdate();
  event.returnValue = 'Downloading update';
});
